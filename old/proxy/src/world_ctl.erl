...
process(["version"]) ->
	...
	?STATUS_SUCCESS;
process(["stop"]) ->
    case stop_mgr:stop_server(180) of
        success -> io:format("stop success~n", []);
        _ -> io:format("stop failed~n", [])
    end,
    ?STATUS_SUCCESS;
process(["stop", Timeout|_]) ->
    case stop_mgr:stop_server(world_util:to_integer(Timeout)) of
        success -> io:format("stop success~n", []);
        _ -> io:format("stop failed~n", [])
    end,
    ?STATUS_SUCCESS;
...
process(["stop_gs"]) ->
    io:format("wait...~n", []),
    gateway_manager:client_disconnect(),
    io:format("stop success~n", []),
    ?STATUS_SUCCESS;
